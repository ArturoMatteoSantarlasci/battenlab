...
const CompositeCalculator: React.FC = () => {
    ...
    const handleSegmentChange = (idx: number, field: keyof BattenSegment, value: string) => {
        if (value === '') {
            updateSegment(idx, field, NaN);
            return;
        }
        ...
        if (segments[idx][field] !== numericValue) {
            updateSegment(idx, field, numericValue);
        }
    };

    const handleSegmentBlur = (idx: number, field: keyof BattenSegment) => {
        const currentVal = segments[idx][field];
        if (isNaN(currentVal)) {
            updateSegment(idx, field, 0);
        }
    };
                        ...
                        {segments.slice(0, numSegments).map((s, idx) => (
                            <div key={idx} className="p-4 bg-slate-50 border border-slate-200 rounded-xl grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    ...
                                    <input
                                        type="number"
                                        value={getValue(s.length)}
                                        onChange={e => handleSegmentChange(idx, 'length', e.target.value)}
                                        onBlur={() => handleSegmentBlur(idx, 'length')}
                                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg font-bold outline-none focus:ring-2 focus:ring-[#A12B2B]"
                                    />
                                </div>
                                <div className="space-y-1">
                                    ...
                                    <input
                                        type="number"
                                        value={getValue(s.ei)}
                                        onChange={e => handleSegmentChange(idx, 'ei', e.target.value)}
                                        onBlur={() => handleSegmentBlur(idx, 'ei')}
                                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg font-bold outline-none focus:ring-2 focus:ring-[#A12B2B]"
                                    />
                                </div>
                            </div>
